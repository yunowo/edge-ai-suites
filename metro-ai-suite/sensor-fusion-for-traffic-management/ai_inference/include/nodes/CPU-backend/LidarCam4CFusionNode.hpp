...
#ifndef __LIDAR_CAMERA_4CFUSION_NODE_H_
#define __LIDAR_CAMERA_4CFUSION_NODE_H_
...
#define LIDAR_CAMERA_4CFUSION_MODULE_INPORT_NUM 5
...
struct lidarCamera4CFusionInPortsInfo_t
{
    int mediaInputPorts[4] = {0, 1, 2, 3};     // in port index for media input
    int lidarInputPort = 4;                    // in port index for lidar input
    int mediaBlobBuffIndex[4] = {0, 0, 0, 0};  // buffer index for media blob
    int lidarBlobBuffIndex = 0;                // buffer index for lidar blob
};

class LidarCam4CFusionNode : public hva::hvaNode_t {
  public:
    LidarCam4CFusionNode(std::size_t totalThreadNum);

    virtual ~LidarCam4CFusionNode();
...
class LidarCam4CFusionNodeWorker : public hva::hvaNodeWorker_t {
  public:
    LidarCam4CFusionNodeWorker(hva::hvaNode_t *parentNode, const int32_t &inMediaNum, const lidarCamera4CFusionInPortsInfo_t &lidarCamera4CFusionInPortsInfo);

    virtual ~LidarCam4CFusionNodeWorker();

    void init() override;

    virtual const std::string nodeClassName() const
    {
        return "LidarCam4CFusionNode";
    };
