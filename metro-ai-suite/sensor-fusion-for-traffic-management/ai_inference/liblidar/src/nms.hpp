...
class NMS {
  ...
  public:
     ...
     */
    NMS(const int num_threads, const int num_box_corners, const float nms_overlap_threshold, DeviceManager *dev_mgr);
     ...
     */
    void DoNMS(const size_t host_filter_count, float *dev_sorted_box_for_nms, int *out_keep_inds, size_t &out_num_to_keep);
  ...
  private:
     ...
     */
    void ParallelNMS(const size_t host_filter_count, float *dev_sorted_box_for_nms, int *out_keep_inds, size_t &out_num_to_keep);

    /**
     * @brief Sequential Non-Maximum Suppresion for network output in SYCL CPU or Host device
     */
    void SequentialNMS(const size_t host_filter_count, float *dev_sorted_box_for_nms, int *out_keep_inds, size_t &out_num_to_keep);
};
}  // namespace pointpillars
