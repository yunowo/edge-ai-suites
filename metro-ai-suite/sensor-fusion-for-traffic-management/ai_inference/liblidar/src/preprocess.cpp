...
void MakePillarHistoKernel(const float *dev_points,
                           ...
                           sycl::nd_item<3> item_ct1)
{
    ...
    if (xIndex >= 0 && xIndex < grid_x_size && yIndex >= 0 && yIndex < grid_y_size && zIndex >= 0 && zIndex < grid_z_size) {
...
void MakePillarIndexKernel(int *dev_pillar_count_histo,
                           int *dev_counter,
...
void MakePillarFeatureKernel(float *dev_pillar_x_in_coors,
                             float *dev_pillar_y_in_coors,
                             float *dev_pillar_z_in_coors,
                             float *dev_pillar_i_in_coors,
                             float *dev_pillar_x,
...
void MakeExtraNetworkInputKernel(float *dev_x_coors_for_sub,
                                 float *dev_y_coors_for_sub,
                                 float *dev_num_points_per_pillar,
...
PreProcess::PreProcess(const int max_num_pillars,
    ...
    : max_num_pillars_(max_num_pillars), max_num_points_per_pillar_(max_points_per_pillar), grid_x_size_(grid_x_size), grid_y_size_(grid_y_size),
...
void PreProcess::DoPreProcess(const float *dev_points,
                              ...
                              int *host_pillar_count)
{
    ...
    queue.submit([&](auto &h) {
        AD(dev_pillar_x_in_coors_);
        AD(dev_pillar_y_in_coors_);
        AD(dev_pillar_z_in_coors_);
        AD(dev_pillar_i_in_coors_);
        AD(dev_pillar_count_histo_);
        AD(max_num_points_per_pillar_);
        AD(grid_x_size_);
        AD(grid_y_size_);
        AD(grid_z_size_);
        AD(min_x_range_);
        ...
        AD(pillar_y_size_);
        AD(pillar_z_size_);
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, num_block) * sycl::range<3>(1, 1, 256), sycl::range<3>(1, 1, 256)),
                       [=](sycl::nd_item<3> item_ct1) {
                           MakePillarHistoKernel(dev_points, AR(dev_pillar_x_in_coors_), AR(dev_pillar_y_in_coors_), AR(dev_pillar_z_in_coors_),
                                                 AR(dev_pillar_i_in_coors_), AR(dev_pillar_count_histo_), in_num_points, AR(max_num_points_per_pillar_),
                                                 AR(grid_x_size_), AR(grid_y_size_), AR(grid_z_size_), AR(min_x_range_), AR(min_y_range_), AR(min_z_range_),
                                                 AR(pillar_x_size_), AR(pillar_y_size_), AR(pillar_z_size_), item_ct1);
                       });
    });
    queue.wait();
    ...
    queue.submit([&](auto &h) {
        AD(dev_pillar_count_histo_);
        AD(dev_counter_);
        AD(dev_pillar_count_);
        AD(dev_x_coors_for_sub_);
        AD(dev_y_coors_for_sub_);
        AD(max_num_pillars_);
        AD(max_num_points_per_pillar_);
        AD(grid_x_size_);
        AD(min_x_range_);
        ...
        AD(pillar_x_size_);
        AD(pillar_y_size_);
        h.parallel_for(sycl::range<2>{static_cast<unsigned long>(grid_x_size_), static_cast<unsigned long>(grid_y_size_)}, [=](sycl::id<2> it) {
            const int x = it[0];
            const int y = it[1];
            MakePillarIndexKernel(AR(dev_pillar_count_histo_), AR(dev_counter_), AR(dev_pillar_count_), dev_x_coors, dev_y_coors, AR(dev_x_coors_for_sub_),
                                  AR(dev_y_coors_for_sub_), dev_num_points_per_pillar, dev_sparse_pillar_map, AR(max_num_pillars_),
                                  AR(max_num_points_per_pillar_), AR(grid_x_size_), AR(min_x_range_), AR(min_y_range_), AR(pillar_x_size_), AR(pillar_y_size_),
                                  x, y);
        });
    });
    queue.wait();
    ...
    queue.submit([&](auto &h) {
        ...
        AD(max_num_points_per_pillar_);
        AD(grid_x_size_);
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, host_pillar_count[0]) * sycl::range<3>(1, 1, max_num_points_per_pillar_),
                                         sycl::range<3>(1, 1, max_num_points_per_pillar_)),
                       [=](sycl::nd_item<3> item_ct1) {
                           MakePillarFeatureKernel(AR(dev_pillar_x_in_coors_), AR(dev_pillar_y_in_coors_), AR(dev_pillar_z_in_coors_),
                                                   AR(dev_pillar_i_in_coors_), dev_pillar_x, dev_pillar_y, dev_pillar_z, dev_pillar_i, dev_x_coors, dev_y_coors,
                                                   dev_num_points_per_pillar, AR(max_num_points_per_pillar_), AR(grid_x_size_), item_ct1);
                       });
    });
    queue.wait();
    ...
    queue.submit([&](auto &h) {
        ...
        AD(dev_y_coors_for_sub_);
        AD(max_num_points_per_pillar_);
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, max_num_pillars_) * sycl::range<3>(1, 1, max_num_points_per_pillar_),
                                         sycl::range<3>(1, 1, max_num_points_per_pillar_)),
                       [=](sycl::nd_item<3> item_ct1) {
                           MakeExtraNetworkInputKernel(AR(dev_x_coors_for_sub_), AR(dev_y_coors_for_sub_), dev_num_points_per_pillar,
                                                       dev_x_coors_for_sub_shaped, dev_y_coors_for_sub_shaped, dev_pillar_feature_mask,
                                                       AR(max_num_points_per_pillar_), item_ct1);
                       });
    });
    queue.wait();
}
