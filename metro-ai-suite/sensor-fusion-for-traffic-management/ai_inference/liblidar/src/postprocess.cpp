...
#include <algorithm>
#include <postprocess.hpp>  // the oneapi headers have to be included at first here!
...
void FilterKernel(const float *box_preds,
                  ...
                  float *multiclass_score,
                  int *filtered_dir,
                  int *dev_filtered_class_id,
                  float *box_for_nms,
                  int *filter_count,
                  ...
                  const int index)
{
    ...
    if (anchor_mask[tid] == 1 && score > score_threshold) {
        ...
        float corners[NUM_3D_BOX_CORNERS_MACRO] = {float(-0.5f * box_dx), float(-0.5f * box_dy), float(-0.5f * box_dx), float(0.5f * box_dy),
                                                   float(0.5f * box_dx),  float(0.5f * box_dy),  float(0.5f * box_dx),  float(-0.5f * box_dy)};
...
void SortBoxesByIndexKernel(float *filtered_box,
                            int *filtered_dir,
                            int *filtered_class_id,
                            float *dev_multiclass_score,
                            float *box_for_nms,
                            int *indexes,
                            int filter_count,
                            float *sorted_filtered_boxes,
                            int *sorted_filtered_dir,
                            int *sorted_filtered_class_id,
                            float *dev_sorted_multiclass_score,
                            float *sorted_box_for_nms,
                            const size_t num_box_corners,
                            const size_t num_output_box_feature,
                            const size_t num_cls,
...
PostProcess::PostProcess(const float float_min,
                         const float float_max,
                         const size_t num_anchor_x_inds,
                         const size_t num_anchor_y_inds,
                         const size_t num_anchor_r_inds,
                         const size_t num_cls,
                         const float score_threshold,
    ...
    : float_min_(float_min), float_max_(float_max), num_anchor_x_inds_(num_anchor_x_inds), num_anchor_y_inds_(num_anchor_y_inds),
      num_anchor_r_inds_(num_anchor_r_inds), num_cls_(num_cls), score_threshold_(score_threshold), num_threads_(num_threads), num_box_corners_(num_box_corners),
      num_output_box_feature_(num_output_box_feature), dev_mgr_(dev_mgr)
{
    nms_ptr_ = std::make_unique<NMS>(num_threads, num_box_corners, nms_overlap_threshold, dev_mgr_);
}

void PostProcess::DoPostProcess(const float *rpn_box_output,
                                ...
                                std::vector<ObjectDetection> &detections)
{
    ...
    queue.submit([&](auto &h) {
        ...
        h.parallel_for(sycl::range<1>{length}, [=](sycl::item<1> it) {
            ...
            FilterKernel(rpn_box_output, rpn_cls_output, rpn_dir_output, dev_anchor_mask, dev_anchors_px, dev_anchors_py, dev_anchors_pz, dev_anchors_dx,
                         dev_anchors_dy, dev_anchors_dz, dev_anchors_ro, dev_filtered_box, dev_filtered_score, dev_multiclass_score, dev_filtered_dir,
                         dev_filtered_class_id, dev_box_for_nms, dev_filter_count, float_min_ct18, float_max_ct19, score_threshold_ct20, num_box_corners_ct21,
    ...
    std::sort(oneapi::dpl::execution::make_device_policy(queue), first, last, [](auto lhs, auto rhs) { return std::get<0>(lhs) > std::get<0>(rhs); });
    ...
    queue.submit([&](auto &h) {
        ...
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) * sycl::range<3>(1, 1, num_threads_), sycl::range<3>(1, 1, num_threads_)),
                       [=](sycl::nd_item<3> item_ct1) {
                           SortBoxesByIndexKernel(dev_filtered_box, dev_filtered_dir, dev_filtered_class_id, dev_multiclass_score, dev_box_for_nms, dev_indexes,
                                                  host_filter_count_ct6, dev_sorted_filtered_box, dev_sorted_filtered_dir, dev_sorted_filtered_class_id,
                                                  dev_sorted_multiclass_score, dev_sorted_box_for_nms, num_box_corners_ct12, num_output_box_feature_ct13,
    ...
    // Copy memory to host
    queue.memcpy(host_filtered_box, dev_sorted_filtered_box, num_output_box_feature_ * host_filter_count[0] * sizeof(float));
    ...
    sycl::free(dev_sorted_filtered_class_id, queue);
    sycl::free(dev_sorted_box_for_nms, queue);
    sycl::free(dev_sorted_multiclass_score, queue);
}
}  // namespace pointpillars
