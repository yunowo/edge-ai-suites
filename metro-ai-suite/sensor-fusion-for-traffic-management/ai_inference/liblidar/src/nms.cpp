...
NMS::NMS(const int num_threads, const int num_box_corners, const float nms_overlap_threshold, DeviceManager *dev_mgr)
    : num_threads_(num_threads), num_box_corners_(num_box_corners), nms_overlap_threshold_(nms_overlap_threshold), dev_mgr_(dev_mgr)
{
}

void NMS::DoNMS(const size_t host_filter_count, float *dev_sorted_box_for_nms, int *out_keep_inds, size_t &out_num_to_keep)
{
...
void NMS::SequentialNMS(const size_t host_filter_count, float *dev_sorted_box_for_nms, int *out_keep_inds, size_t &out_num_to_keep)
{
    ...
    for (size_t i = 0; i < host_filter_count; ++i) {
        for (size_t j = i + 1; j < host_filter_count; ++j) {
            auto iou_value = DevIoU(dev_sorted_box_for_nms + i * num_box_corners_, dev_sorted_box_for_nms + j * num_box_corners_);
            ...
            float *block_boxes)
{
    ...
    if (item_ct1.get_local_id(2) < col_size) {
        block_boxes[item_ct1.get_local_id(2) * num_box_corners + 0] = dev_boxes[(block_threads * col_start + item_ct1.get_local_id(2)) * num_box_corners + 0];
        block_boxes[item_ct1.get_local_id(2) * num_box_corners + 1] = dev_boxes[(block_threads * col_start + item_ct1.get_local_id(2)) * num_box_corners + 1];
        block_boxes[item_ct1.get_local_id(2) * num_box_corners + 2] = dev_boxes[(block_threads * col_start + item_ct1.get_local_id(2)) * num_box_corners + 2];
        block_boxes[item_ct1.get_local_id(2) * num_box_corners + 3] = dev_boxes[(block_threads * col_start + item_ct1.get_local_id(2)) * num_box_corners + 3];
    }

    if (item_ct1.get_local_id(2) < row_size) {
        const int cur_box_idx = block_threads * row_start + item_ct1.get_local_id(2);
        const float cur_box[NUM_2D_BOX_CORNERS_MACRO] = {dev_boxes[cur_box_idx * num_box_corners + 0], dev_boxes[cur_box_idx * num_box_corners + 1],
...
void NMS::ParallelNMS(const size_t host_filter_count, float *dev_sorted_box_for_nms, int *out_keep_inds, size_t &out_num_to_keep)
{
    ...
    queue.submit([&](auto &h) {
        ...
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(global_range.get(2), global_range.get(1), global_range.get(0)),
                       ...
                       [=](sycl::nd_item<3> item_ct1) {
                           Kernel(host_filter_count, nms_overlap_threshold_ct1, dev_sorted_box_for_nms, dev_mask, num_box_corners_ct4, item_ct1,
                                  block_boxes_acc_ct1.get_pointer());
                       });
    });
    queue.wait();
