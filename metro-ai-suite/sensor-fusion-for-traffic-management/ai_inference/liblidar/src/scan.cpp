...
void ScanX(int *dev_output, const int *dev_input, int w, int h, int n, DeviceManager *dev_mgr)
{
    ...
    if (!dev_mgr->getDev().is_cpu()) {
        ...
        queue.submit([&](sycl::handler &cgh) {
            sycl::accessor<uint8_t, 1, sycl::access::mode::read_write, sycl::access::target::local> local_acc_ct1(sycl::range<1>(n * sizeof(int)), cgh);
            cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, h) * sycl::range<3>(1, 1, w / 2), sycl::range<3>(1, 1, w / 2)),
                             [=](sycl::nd_item<3> item_ct1) { ScanXKernel(dev_output, dev_input, n, item_ct1, local_acc_ct1.get_pointer()); });
        });
    }
...
void ScanY(int *dev_output, const int *dev_input, int w, int h, int n, DeviceManager *dev_mgr)
{
    ...
    if (!dev_mgr->getDev().is_cpu()) {
        ...
        queue.submit([&](sycl::handler &cgh) {
            sycl::accessor<uint8_t, 1, sycl::access::mode::read_write, sycl::access::target::local> local_acc_ct1(sycl::range<1>(n * sizeof(int)), cgh);
            cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, w) * sycl::range<3>(1, 1, h / 2), sycl::range<3>(1, 1, h / 2)),
                             [=](sycl::nd_item<3> item_ct1) { ScanYKernel(dev_output, dev_input, n, item_ct1, local_acc_ct1.get_pointer()); });
        });
    }
