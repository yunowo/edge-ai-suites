...
AnchorGrid::AnchorGrid(AnchorGridConfig &config, DeviceManager *dev_mgr)
    : config_{config}, dev_mgr_{dev_mgr}, dev_anchors_px_{nullptr}, dev_anchors_py_{nullptr}, dev_anchors_pz_{nullptr}, dev_anchors_dx_{nullptr},
      dev_anchors_dy_{nullptr}, dev_anchors_dz_{nullptr}, dev_anchors_ro_{nullptr}, dev_anchors_rad_{nullptr}, host_anchors_px_{nullptr},
      host_anchors_py_{nullptr}, host_anchors_pz_{nullptr}, host_anchors_dx_{nullptr}, host_anchors_dy_{nullptr}, host_anchors_dz_{nullptr},
      host_anchors_ro_{nullptr}, host_anchors_rad_{nullptr}
{
...
void AnchorGrid::CreateAnchorMask(int *dev_pillar_map,
                                  const int pillar_map_w,
                                  ...
                                  int *dev_pillar_workspace)
{
    ...
    ScanX(dev_pillar_workspace, dev_pillar_map, pillar_map_h, pillar_map_w, n, dev_mgr_);
    ScanY(dev_pillar_map, dev_pillar_workspace, pillar_map_h, pillar_map_w, n, dev_mgr_);
    // Mask anchors only where input data is found
    MaskAnchors(dev_anchors_px_, dev_anchors_py_, dev_pillar_map, dev_anchor_mask, dev_anchors_rad_, config_.min_x_range, config_.min_y_range, pillar_size_x,
                pillar_size_y, pillar_map_h, pillar_map_w, mc_, mr_, mh_, mw_);
}

void MaskAnchorsKernel(const float *anchors_px,
                       const float *anchors_py,
                       const int *pillar_map,
                       int *anchor_mask,
                       const float *anchors_rad,
...
void MaskAnchorsSimpleKernel(const float *anchors_px,
                             const float *anchors_py,
                             const int *pillar_map,
                             int *anchor_mask,
                             const float *anchors_rad,
                             ...
                             const int W)
{
    ...
    if (!dev_mgr_->getDev().is_gpu()) {
        ...
        queue.submit([&](auto &h) {
            ...
            h.parallel_for(
                sycl::nd_range<3>(sycl::range<3>(range.get(2), range.get(1), range.get(0)), sycl::range<3>(block.get(2), block.get(1), block.get(0))),
                [=](sycl::nd_item<3> item_ct1) {
                    MaskAnchorsKernel(dev_anchors_px, dev_anchors_py, dev_pillar_map, dev_anchor_mask, dev_anchors_rad, min_x_range, min_y_range, pillar_x_size,
                                      pillar_y_size, grid_x_size, grid_y_size, item_ct1);
                });
        });
    }
    else {
        ...
        queue.submit([&](auto &h) {
            h.parallel_for(sycl::range<1>{length}, [=](sycl::id<1> it) {
                ...
                MaskAnchorsSimpleKernel(dev_anchors_px, dev_anchors_py, dev_pillar_map, dev_anchor_mask, dev_anchors_rad, min_x_range, min_y_range,
                                        pillar_x_size, pillar_y_size, grid_x_size, grid_y_size, index, c);
            });
        });
    }
