...
void ScatterKernel(int *x_coors,
                   int *y_coors,
                   float *pfe_output,
...
Scatter::Scatter(const int num_features, const int max_num_pillars, const int grid_x_size, const int grid_y_size, DeviceManager *dev_mgr)
    : num_features_(num_features), max_num_pillars_(max_num_pillars), grid_x_size_(grid_x_size), grid_y_size_(grid_y_size), dev_mgr_(dev_mgr)
{
}
...
void Scatter::DoScatter(const int pillar_count, int *x_coors, int *y_coors, float *pfe_output, float *scattered_feature)
{
    ...
    queue.submit([&](auto &h) {
        ...
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(1, 1, pillar_count) * sycl::range<3>(1, 1, num_features_), sycl::range<3>(1, 1, num_features_)),
                       [=](sycl::nd_item<3> item_ct1) {
                           ScatterKernel(x_coors, y_coors, pfe_output, scattered_feature, max_num_pillars_ct4, grid_x_size_ct5, grid_y_size_ct6, item_ct1);
                       });
    });
    queue.wait();
}
