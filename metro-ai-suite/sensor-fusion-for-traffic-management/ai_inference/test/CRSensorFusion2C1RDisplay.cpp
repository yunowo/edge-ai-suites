...
void parseReply(cv::Mat &frame, Plot &pl, float resize_rate, std::string imsg, int frameCnt, std::string display_type)
{
            ...
            for (auto roi_i = roi_info.begin(); roi_i != roi_info.end(); ++roi_i) {
                ...
                if ("radar" != display_type) {
                    ...
                    if ((x == 0 && y == 0 && w == 0 && h == 0) || (-1 == sensor_source)) {
                        ...
                        cv::Point frame1_start_point(x_resize + sensor_source * raw_image_width, y_resize);
                        cv::Point frame1_start_point2(x_resize + sensor_source * raw_image_width, max(10, y_resize - 6));
                        cv::Point frame1_end_point(x_resize + w_resize + sensor_source * raw_image_width, y_resize + h_resize);

                        char objectStr[256];
                        sprintf(objectStr, "[%s]: %0.2f", roi_class_name.c_str(), roi_score);
                        // draw results
                        cv::putText(frame, objectStr, frame1_start_point2, cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar{0, 127, 255}, 1, cv::LINE_8);
                        cv::rectangle(frame, frame1_start_point, frame1_end_point, cv::Scalar{0, 255, 0}, 1, cv::LINE_8);
                    }
    ...
    cv::putText(frame, fpsStr, cv::Point(frame.cols - textSize.width - 10, offset), font, fontScale, {0, 255, 0}, thickness, cv::LINE_8);
    offset += textSize.height + baseline + lineSpace;

    textSize = cv::getTextSize(frameCntStr, font, fontScale, thickness, &baseline);
    cv::putText(frame, frameCntStr, cv::Point(frame.cols - textSize.width - 10, offset), font, fontScale, {0, 255, 0}, thickness, cv::LINE_8);
    offset += baseline + textSize.height + lineSpace;

    // show latency
    textSize = cv::getTextSize(latencyStrStd, font, fontScale, thickness, &baseline);
    cv::putText(frame, latencyStrStd, cv::Point(frame.cols - textSize.width - 10, offset), font, fontScale, {0, 255, 0}, thickness, cv::LINE_8);
}
...
void plotMetrics(cv::Mat &image, int frame_index, int cols)
{
    if (!stop_metrics) {
        ...
        cv::putText(image, "Intel Distribution of OpenVINO (2025.2.0)", cv::Point(image.cols - cols - textSize.width, offset), font, fontScale, {255, 0, 0},
                    thickness, cv::LINE_8);
        offset += textSize.height + baseline + lineSpace;

        textSize = cv::getTextSize("Intel oneMKL (2025.2.0)", font, fontScale, thickness, &baseline);
        cv::putText(image, "Intel oneMKL (2025.2.0)", cv::Point(image.cols - cols - textSize.width, offset), font, fontScale, {255, 0, 0}, thickness,
                    cv::LINE_8);
        offset += textSize.height + baseline + lineSpace;
    ...
    try {
        ...
        if (argc < 8 || argc > 14) {
            std::cerr << "Usage: CRSensorFusionDisplay <host> <port> <json_file> <total_stream_num> <repeats> <data_path> <display_type> "
                      ...
                      << "Example:\n"
                      << "    ./CRSensorFusion2C1RDisplay 127.0.0.1 50052 ../../ai_inference/test/configs/raddet/2C1R/localFusionPipeline_libradar.json 1 1 "
