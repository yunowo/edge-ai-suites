...
void plotMetrics(cv::Mat &image, int frame_index)
{
    if (!stop_metrics) {
        ...
        offset += lineHeight;
        cv::putText(image, "Intel oneMKL (2025.2.0)", cv::Point(50, offset), font, fontScale, {255, 0, 0}, thickness, cv::LINE_8);
        offset += lineHeight;
        cv::putText(image, "Intel Distribution of OpenVINO (2025.2.0)", cv::Point(50, offset), font, fontScale, {255, 0, 0}, thickness, cv::LINE_8);
    }
}
...
void renderOSDImage(cv::Mat raw_image, cv::Mat &osd_image, int frame_index, std::string display_type, MessageBuffer_t *db, Plot &pl, bool show_metrics = false)
{
    ...
    if (display_type == "tracking") {
        ...
        parsePCL(osd_image, pl, resize_rate, replyGet(db), frame_index, display_type);
    }
    else if (display_type == "clustering") {
        parseClustering(osd_image, pl, resize_rate, replyGet(db), frame_index, display_type);
    }
...
int main(int argc, char **argv)
{
    try {
        ...
        if (argc != 8 && argc != 9 && argc != 10) {
            std::cerr << "Usage: CRSensorFusionRadarDisplay <host> <port> <json_file> <total_stream_num> <repeats> <data_path> <media_type> "
                         "[<cross_stream_num>] [<warmup_flag: 0 | 1>]\n"
                      << "Example:\n"
                      << "    ./CRSensorFusionRadarDisplay 127.0.0.1 50052 ../../ai_inference/test/configs/raddet/localRadarPipeline_savepcl.json 1 1 "
                         "/path/to/dataset pcl \n"
                      << "Or:\n"
                      << "    ./CRSensorFusionRadarDisplay 127.0.0.1 50052 ../../ai_inference/test/configs/raddet/localRadarPipeline_saveClustering.json 1 1 "
                         "/path/to/dataset clustering \n"
                      << "Or:\n"
                      << "    ./CRSensorFusionRadarDisplay 127.0.0.1 50052 ../../ai_inference/test/configs/raddet/localRadarPipeline.json 1 1 /path/to/dataset "
                         "tracking \n"
                      << "-------------------------------------------------------------------------------- \n"
        ...
        // for(unsigned i = 0; i < threadNum; ++i){
        //     initMessageBuffer(&msg_buffer[i]);
        //     std::thread t(workload, std::ref(host), std::ref(port), std::ref(contents), std::ref(inputs), i, &msg_buffer[i], fpsWindow);  //workload for ai
        //     inference vThs.push_back(std::move(t));
        // }
        sleep(2);
        ...
        for (unsigned j = 0; j < threadNum; ++j) {
            ...
            std::thread d(OSDload, std::ref(repeatinputs), j, displayType, windowImage, params, &msg_buffer[j], std::ref(pl), mediaType, showMetrics, saveFlag,
                          logoFlag);
