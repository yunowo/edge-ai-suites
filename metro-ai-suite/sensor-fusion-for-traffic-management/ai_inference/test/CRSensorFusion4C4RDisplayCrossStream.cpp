...
void workload(const std::string &host,
              ...
              unsigned repeats,
              std::vector<MessageBuffer_t *> &pbs,
              unsigned thread_id,
              unsigned stream_num,
        ...
        std::thread reader([&]() {
            ...
            while (stream->Read(&reply)) {
                ...
                try {
                    ...
                    while (((pbs[stream_id]->write_pos + 1) % MAX_MESSAGE_BUFFER_SIZE) == pbs[stream_id]->read_pos) {
                        // buffer is full
...
void plotMetrics(cv::Mat &image, int frame_index, int cols)
{
    if (!stop_metrics) {
        ...
        // show metrics
        //
        textSize = cv::getTextSize("Intel Distribution of OpenVINO (2025.2.0)", font, fontScale, thickness, &baseline);
        cv::putText(image, "Intel Distribution of OpenVINO (2025.2.0)", cv::Point(image.cols - cols + 100, offset), font, fontScale, {0, 127, 255}, thickness,
                    cv::LINE_8);
        offset += textSize.height + baseline + lineSpace;

        textSize = cv::getTextSize("Intel oneMKL (2025.2.0)", font, fontScale, thickness, &baseline);
        cv::putText(image, "Intel oneMKL (2025.2.0)", cv::Point(image.cols - cols + 100, offset), font, fontScale, {0, 127, 255}, thickness, cv::LINE_8);
        offset += textSize.height + baseline + lineSpace;
...
int main(int argc, char **argv)
{
    try {
        ...
        DisplayParams_t params = prepareDisplayParams(fusionType, totalStreamNum, WINDOWNAME, pl);
        ...
        //                                 processing
        //  ---------------------------------------------------------------------------
        std::cout << "Start processing with " << all_threadNum << " threads: total-stream = " << totalStreamNum << ", each thread will process "
        ...
        for (unsigned i = 0; i < gpu_threadNum; i++) {
            for (unsigned j = i * crossStreamNum; j < crossStreamNum; j++) {
                MessageBuffer_t *newMsgBuf = (MessageBuffer_t *)malloc(sizeof(MessageBuffer_t));
                initMessageBuffer(newMsgBuf);
                msg_buffer_gpu.emplace_back(newMsgBuf);
            }
            std::thread t(workload, std::ref(host), std::ref(port), std::ref(contents), std::ref(inputs), repeats, std::ref(msg_buffer_gpu), i, crossStreamNum,
        ...
        for (unsigned i = 0; i < npu_threadNum; i++) {
            ...
            std::thread t(workload, std::ref(host), std::ref(port), std::ref(additionalContents), std::ref(inputs), repeats, std::ref(msg_buffer_npu), i,
                          npu_crossStreamNum, pipeline_repeats, warmupFlag);
            vThs.push_back(std::move(t));
        }
        ...
        for (unsigned j = 0; j < gpu_streams; ++j) {
            ...
            std::thread d(OSDload, std::ref(pipeline_repeatinputs), totalStreamNum, display_idx, windowImage, params, msg_buffer_gpu[j], std::ref(pl),
        ...
        for (unsigned j = 0; j < npu_streams; ++j) {
            ...
            std::thread d(OSDload, std::ref(pipeline_repeatinputs), totalStreamNum, display_idx, windowImage, params, msg_buffer_npu[j], std::ref(pl),
                          mediaType, displayType, showMetrics, saveFlag, logoFlag);
            dThs.push_back(std::move(d));
            display_idx++;
        }
