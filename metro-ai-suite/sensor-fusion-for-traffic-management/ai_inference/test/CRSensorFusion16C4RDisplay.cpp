    ...
    if (jsonTree.count("latency") > 0) {
        ...
        for (auto roi_i = roi_info.begin(); roi_i != roi_info.end(); ++roi_i) {
            ...
            if ((x == 0 && y == 0 && w == 0 && h == 0) || (-1 == sensor_source)) {
                ...
                cv::Point start_point(x_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4),
                                      y_resize + raw_image_height * ((thread_id * 4 + sensor_source) % 4));
                cv::Point end_point(x_resize + w_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4),
                                    y_resize + h_resize + raw_image_height * ((thread_id * 4 + sensor_source) % 4));
                cv::Point start_point2(x_resize + raw_image_width * ((thread_id * 4 + sensor_source) / 4),
                                       max(10, y_resize - 6) + raw_image_height * ((thread_id * 4 + sensor_source) % 4));
                    ...
                    bool show_metrics = false)
{
    ...
    cv::Mat plot = pl.getPlotImage();
    cv::resize(raw_image, raw_image, Size(display_params.frameSize.width, display_params.frameSize.height));
    cv::resize(plot, plot, Size(plot.cols * 2, plot.rows * 2));
    // cv::hconcat(raw_image, plot, osd_image);

    std::lock_guard<std::mutex> lg(g_window_mutex);
    for (int i = thread_id * 4; i < (thread_id + 1) * 4; i++) {
        raw_image.copyTo(windowImage(cv::Rect(display_params.points[i].x, display_params.points[i].y, raw_image.cols, raw_image.rows)));
    }
...
int main(int argc, char **argv)
{
    try {
        ...
        if (argc < 8 || argc > 13) {
            std::cerr << "Usage: CRSensorFusion16C4RDisplay <host> <port> <json_file> <total_stream_num> <repeats> <data_path> <display_type> "
                         "[<save_flag: 0 | 1>] [<pipeline_repeats>] [<cross_stream_num>] [<warmup_flag: 0 | 1>] [<logo_flag: 0 | 1>]\n"
                      << "Example:\n"
                      << "    ./CRSensorFusion16C4RDisplay 127.0.0.1 50052 ../../ai_inference/test/configs/raddet/16C4R/localFusionPipeline.json 4 1 "
        ...
        DisplayParams_t params = prepareDisplayParams(fusionType, 16, WINDOWNAME, pl);
