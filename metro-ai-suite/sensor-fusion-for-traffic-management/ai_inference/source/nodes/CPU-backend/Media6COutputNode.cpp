...
#include "nodes/CPU-backend/Media6COutputNode.hpp"
...
Media6COutputNodeWorker::Media6COutputNodeWorker(hva::hvaNode_t *parentNode, const std::string &bufType) : hva::hvaNodeWorker_t(parentNode), m_bufType(bufType)
{
}

void Media6COutputNodeWorker::process(std::size_t batchIdx)
{
    std::vector<size_t> portIndices;
    for (size_t portId = 0; portId < MEDIA_6C_OUTPUT_NODE_INPORT_NUM; portId++) {
    ...
    if (vecBlobInput.size() > 0) {
        ...
        dynamic_cast<Media6COutputNode *>(getParentPtr())->emitOutput(res, (baseResponseNode *)getParentPtr(), nullptr);

        if (mediaBuf->getTag() == 1) {
            HVA_DEBUG("Emit finish on frame id %d", mediaBuf->frameId);
            dynamic_cast<Media6COutputNode *>(getParentPtr())->emitFinish((baseResponseNode *)getParentPtr(), nullptr);
        }
    }
}

void Media6COutputNodeWorker::processByLastRun(std::size_t batchIdx)
{
    dynamic_cast<Media6COutputNode *>(getParentPtr())->emitFinish((baseResponseNode *)getParentPtr(), nullptr);
}

Media6COutputNode::Media6COutputNode(std::size_t totalThreadNum) : baseResponseNode(MEDIA_6C_OUTPUT_NODE_INPORT_NUM, 0, totalThreadNum), m_bufType("FD")
{
    transitStateTo(hva::hvaState_t::configured);
}

hva::hvaStatus_t Media6COutputNode::configureByString(const std::string &config)
{
...
std::shared_ptr<hva::hvaNodeWorker_t> Media6COutputNode::createNodeWorker() const
{
    return std::shared_ptr<hva::hvaNodeWorker_t>(new Media6COutputNodeWorker((hva::hvaNode_t *)this, m_bufType));
}

#ifdef HVA_NODE_COMPILE_TO_DYNAMIC_LIBRARY
HVA_ENABLE_DYNAMIC_LOADING(Media6COutputNode, Media6COutputNode(threadNum))
