...
#include "nodes/CPU-backend/LidarCam3CFusionNode.hpp"
...
class LidarCam3CFusionNode::Impl {
  public:
    Impl(LidarCam3CFusionNode &ctx);
     ...
     */
    std::shared_ptr<hva::hvaNodeWorker_t> createNodeWorker(LidarCam3CFusionNode *parent) const;
  ...
  private:
    LidarCam3CFusionNode &m_ctx;
    hva::hvaConfigStringParser_t m_configParser;
    lidarCamera3CFusionInPortsInfo_t m_lidarCamera3CFusionInPortsInfo;
    int32_t m_inMediaNum;
};

LidarCam3CFusionNode::Impl::Impl(LidarCam3CFusionNode &ctx) : m_ctx(ctx)
{
    m_configParser.reset();
}

LidarCam3CFusionNode::Impl::~Impl() {}
 ...
 * @return hva status
 */
hva::hvaStatus_t LidarCam3CFusionNode::Impl::configureByString(const std::string &config)
{
    if (config.empty())
 ...
 * @return hva status
 */
hva::hvaStatus_t LidarCam3CFusionNode::Impl::validateConfiguration() const
{
    if (3 != m_inMediaNum) {
        HVA_ERROR("Error inMediaNum, need values 3!");
        return hva::hvaFailure;
    }
 ...
 * @return shared_ptr of hvaNodeWorker
 */
std::shared_ptr<hva::hvaNodeWorker_t> LidarCam3CFusionNode::Impl::createNodeWorker(LidarCam3CFusionNode *parent) const
{
    return std::shared_ptr<hva::hvaNodeWorker_t>(new LidarCam3CFusionNodeWorker(parent, m_inMediaNum, m_lidarCamera3CFusionInPortsInfo));
}

hva::hvaStatus_t LidarCam3CFusionNode::Impl::prepare()
{
    return hva::hvaSuccess;
}

hva::hvaStatus_t LidarCam3CFusionNode::Impl::rearm()
{
    return hva::hvaSuccess;
}

hva::hvaStatus_t LidarCam3CFusionNode::Impl::reset()
{
    return hva::hvaSuccess;
}

LidarCam3CFusionNode::LidarCam3CFusionNode(std::size_t totalThreadNum)
    : hva::hvaNode_t(LIDAR_CAMERA_3CFUSION_MODULE_INPORT_NUM, 1, totalThreadNum), m_impl(new Impl(*this))
{
}

LidarCam3CFusionNode::~LidarCam3CFusionNode() {}
 ...
 * @return hva status
 */
hva::hvaStatus_t LidarCam3CFusionNode::configureByString(const std::string &config)
{
 ...
 * @return hva status
 */
hva::hvaStatus_t LidarCam3CFusionNode::validateConfiguration() const
{
 ...
 * @return shared_ptr of hvaNodeWorker
 */
std::shared_ptr<hva::hvaNodeWorker_t> LidarCam3CFusionNode::createNodeWorker() const
{
    return m_impl->createNodeWorker(const_cast<LidarCam3CFusionNode *>(this));
}

hva::hvaStatus_t LidarCam3CFusionNode::rearm()
{
    return m_impl->rearm();
}

hva::hvaStatus_t LidarCam3CFusionNode::reset()
{
    return m_impl->reset();
}

hva::hvaStatus_t LidarCam3CFusionNode::prepare()
{
    return m_impl->prepare();
}

class LidarCam3CFusionNodeWorker::Impl {
  public:
    Impl(LidarCam3CFusionNodeWorker &ctx, const int32_t &inMediaNum, const lidarCamera3CFusionInPortsInfo_t &lidarCamera3CFusionInPortsInfo);
  ...
  private:
    ...
    LidarCam3CFusionNodeWorker &m_ctx;
    lidarCamera3CFusionInPortsInfo_t m_lidarCamera3CFusionInPortsInfo;
};

LidarCam3CFusionNodeWorker::Impl::Impl(LidarCam3CFusionNodeWorker &ctx,
                                       const int32_t &inMediaNum,
                                       const lidarCamera3CFusionInPortsInfo_t &lidarCamera3CFusionInPortsInfo)
    : m_ctx(ctx), m_lidarCamera3CFusionInPortsInfo(lidarCamera3CFusionInPortsInfo)
{
    m_multiCameraFuser.setNmsThreshold(0.5);
}

LidarCam3CFusionNodeWorker::Impl::~Impl() {}

void LidarCam3CFusionNodeWorker::Impl::init()
{
    return;
}
 ...
 * @param batchIdx Internal parameter handled by hvaframework
 */
void LidarCam3CFusionNodeWorker::Impl::process(std::size_t batchIdx)
{
    std::vector<size_t> portIndices;
    for (size_t portId = 0; portId < LIDAR_CAMERA_3CFUSION_MODULE_INPORT_NUM; portId++) {
    ...
    if (vecBlobInput.size() != 0) {
        if (vecBlobInput.size() != LIDAR_CAMERA_3CFUSION_MODULE_INPORT_NUM) {
            HVA_ERROR("Camera3CFusion node received %d inputs at node %d, but expect to be: %d", vecBlobInput.size(), batchIdx,
                      LIDAR_CAMERA_3CFUSION_MODULE_INPORT_NUM);
            HVA_ASSERT(false);
        }

        hva::hvaBlob_t::Ptr cameraBlob1 = vecBlobInput[m_lidarCamera3CFusionInPortsInfo.mediaInputPorts[0]];
        hva::hvaBlob_t::Ptr lidarBlob = vecBlobInput[m_lidarCamera3CFusionInPortsInfo.lidarInputPort];
        HVA_ASSERT(cameraBlob1);
        HVA_ASSERT(lidarBlob);

        hva::hvaVideoFrameWithROIBuf_t::Ptr ptrFrameBuf1 =
            std::dynamic_pointer_cast<hva::hvaVideoFrameWithROIBuf_t>(cameraBlob1->get(m_lidarCamera3CFusionInPortsInfo.mediaBlobBuffIndex[0]));
        ...
        LidarCameraFusionOutput fusionOutput(LIDAR_CAMERA_3CFUSION_MODULE_INPORT_NUM - 1);
        ...
        for (int i = 1; i < LIDAR_CAMERA_3CFUSION_MODULE_INPORT_NUM - 1; i++) {
            hva::hvaBlob_t::Ptr cameraBlob = vecBlobInput[m_lidarCamera3CFusionInPortsInfo.mediaInputPorts[i]];
            HVA_ASSERT(cameraBlob);
            HVA_DEBUG("LidarCamera3CFusion node %d on frameId %d at port id: %d(Media %d)", batchIdx, cameraBlob->frameId,
                      m_lidarCamera3CFusionInPortsInfo.mediaInputPorts[i], i + 1);

            /**
             * process: media
             */
            hva::hvaVideoFrameWithROIBuf_t::Ptr ptrFrameBuf =
                std::dynamic_pointer_cast<hva::hvaVideoFrameWithROIBuf_t>(cameraBlob->get(m_lidarCamera3CFusionInPortsInfo.mediaBlobBuffIndex[i]));
        ...
        hva::hvaVideoFrameWithMetaROIBuf_t::Ptr ptrLidarBuf =
            std::dynamic_pointer_cast<hva::hvaVideoFrameWithMetaROIBuf_t>(lidarBlob->get(m_lidarCamera3CFusionInPortsInfo.lidarBlobBuffIndex));
        ...
        HVA_DEBUG("LidarCam3CFusionNode sending blob with frameid %u and streamid %u", cameraBlob1->frameId, cameraBlob1->streamId);
        m_ctx.sendOutput(cameraBlob1, 0, std::chrono::milliseconds(0));
        HVA_DEBUG("LidarCam3CFusionNode completed sent blob with frameid %u and streamid %u", cameraBlob1->frameId, cameraBlob1->streamId);
...
hva::hvaStatus_t LidarCam3CFusionNodeWorker::Impl::rearm()
{
    return hva::hvaSuccess;
}

hva::hvaStatus_t LidarCam3CFusionNodeWorker::Impl::reset()
{
    return hva::hvaSuccess;
}

LidarCam3CFusionNodeWorker::LidarCam3CFusionNodeWorker(hva::hvaNode_t *parentNode,
                                                       const int32_t &inMediaNum,
                                                       const lidarCamera3CFusionInPortsInfo_t &lidarCamera3CFusionInPortsInfo)
    : hva::hvaNodeWorker_t(parentNode), m_impl(new Impl(*this, inMediaNum, lidarCamera3CFusionInPortsInfo))
{
}

LidarCam3CFusionNodeWorker::~LidarCam3CFusionNodeWorker() {}

void LidarCam3CFusionNodeWorker::init()
{
    return m_impl->init();
}

void LidarCam3CFusionNodeWorker::process(std::size_t batchIdx)
{
    return m_impl->process(batchIdx);
}

#ifdef HVA_NODE_COMPILE_TO_DYNAMIC_LIBRARY
HVA_ENABLE_DYNAMIC_LOADING(LidarCam3CFusionNode, LidarCam3CFusionNode(threadNum))
