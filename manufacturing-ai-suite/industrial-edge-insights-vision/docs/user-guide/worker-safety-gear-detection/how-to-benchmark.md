# How to Benchmark Worker Safety Gear Detection
This guide demonstrates how to benchmark the worker safety gear detection pipeline to determine optimal stream density and performance characteristics.
...
**Arguments:**
- `-p <pipeline_name>` : **(Required)** The name of the pipeline to benchmark (e.g., worker_safety_gear_detection)
...
3. **Pipeline Selection**: Choose from available pipelines in the payload file (e.g., `worker_safety_gear_detection`, `worker_safety_gear_detection_gpu`)

Available pipelines for worker safety gear detection:
...
2. **Test CPU performance**:
   ```bash
   ./benchmark_start.sh -p worker_safety_gear_detection -l 1 -u 10 -t 25.0 -i 30
   ```

3. **Test GPU performance** (if available):
   ```bash
   ./benchmark_start.sh -p worker_safety_gear_detection_gpu -l 1 -u 20 -t 28.5 -i 60
   ```
...
4. **Debug Mode**
   ...
   ```bash
   ./benchmark_start.sh -p worker_safety_gear_detection_gpu -l 1 -u 10 --trace
   ```
