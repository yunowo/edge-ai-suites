# How to Benchmark Weld Porosity Classification
This guide demonstrates how to benchmark the weld porosity classification pipeline to determine optimal stream density and performance characteristics.
...
**Arguments:**
- `-p <pipeline_name>` : **(Required)** The name of the pipeline to benchmark (e.g., weld_porosity_classification)
...
3. **Pipeline Selection**: Choose from available pipelines in the payload file (e.g., `weld_porosity_classification`, `weld_porosity_classification_gpu`)

Available pipelines for weld porosity classification:
- **`weld_porosity_classification`**: CPU-based classification pipeline
- **`weld_porosity_classification_gpu`**: GPU-accelerated classification pipeline with optimized settings
...
1. **Set up the environment**: Ensure `SAMPLE_APP=weld-porosity` is set in your `.env` file

2. **Test CPU performance**:
   ```bash
   ./benchmark_start.sh -p weld_porosity_classification -l 1 -u 10 -t 25.0 -i 30
   ```

3. **Test GPU performance** (if available):
   ```bash
   ./benchmark_start.sh -p weld_porosity_classification_gpu -l 1 -u 20 -t 28.5 -i 60
   ```
...
4. **Debug Mode**
   ...
   ```bash
   ./benchmark_start.sh -p weld_porosity_classification_gpu -l 1 -u 10 --trace
   ```
